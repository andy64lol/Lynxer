# Lynxer Investigation Report

**Date and Time:** 2026-09-20 20:26 CEST (2026-09-20 18:26 UTC)
**Author:** investigation session (automated)
**Scope:** Lynxer source tree under `lynxer/`, the `sound`/`sqldb`/`tui` stdlib
modules, the native-module ABI, the build and test system, and the state of
`todo.md` and the Lynxer documentation.

> **Build-system note (2026-09-22):** `lynxer/Makefile` has since been merged
> into the root `Makefile`, so every `lynxer/Makefile:NNN` citation below is a
> historical reference to a file that no longer exists. The Lynxer target
> names are unchanged (`make buildLynxer`, `make testLynxer`, `make cargo`),
> and the suite now runs from the repo root.
**Result:** `make buildLynxer` and `make testLynxer` both pass. **Six classes of
...
automated check (`lynxer/scripts/check_module_contracts.py`) guards the two
...
> for Lynxer" exists. Both are contradicted by `todo.md` and by the shipped
...
> AST optimizer (`lynxer/optimizer.cpp`, `--no-opt`, `LYNXER_OPT_REPORT`)
> landed; the CLI/diagnostic golden gate (`lynxer/scripts/check_golden.py`) and
...
>
> **Revision 11 (2026-09-23, later)** records that **Lynxer has surpassed the
...
> reference in `README.md`, `clynxer/__init__.py` and `clynxer/shell.py`. Two
> language bugs found while verifying the Lynxer docs were fixed: compound
...
- Implemented in Python under `clynxer/`. The interpreter pipeline
  (lexer + parser + interpreter) lives in `clynxer/clynxer.py`, builtins in
  `clynxer/builtins.py`, and the CLI in `clynxer/shell.py`.
- Ships a **C++ Python extension** for the native-memory built-ins
  (`clynxer/cpp.cpp`) and a **bytecode stack machine** (`clynxer/bytecode_vm.cpp`)
  compiled by `clynxer/setup.py`.
- **Has bytecode.** `clynxer --compile foo.lynx` writes `foo.lynxc`, which can be
  executed directly; see `docs/bytecode.md`. This is the Python `.lynxc`
  container and is **not** related to anything in Lynxer.
...
- **There is no bytecode backend** (`docs/limitations.md:24-28`).
  ...
  `lynxer/Makefile:223-227` asserts exactly that.
- **No `rawPy`.** Because there is no Python runtime, every Python-backed
  ...
  increasingly Rust.
- Reports version `Lynxer 0.1.8` (`lynxer --version`) and reports itself as
  `"Lynxer"` from `os.getPythonImplementation()` / `sys.implementation()`.
...
| Executable | PyInstaller `dist/clynxer` | `lynxer/lynxer` (ELF) |
...
backend, `clynxer/stdlib/<name>.lynx` (the Python reference) defines the intended
semantics — but the C++ `lynxer/stdlib/<name>.lynx` wrapper defines the
...
| `todo.md` | **The Lynxer roadmap and source of truth for milestone status.** |
...
| `config.cpp` / `lynxer.config` | Version strings and user-facing message templates |
...
### 2.3 `clynxer/` (Python) internals — for reference only

`clynxer.py` (lexer + parser + interpreter + bytecode compiler), `builtins.py`,
`values.py`, `type_registry.py`, `builtin_registry.py`, `runtime.py`,
`runtime_state.py`, `lexer.py`, `parser.py`, `clynxerAst.py`, `bytecode.py`,
...
| see what CI enforces | `.github/workflows/buildLynxer.yml`, `lynxer/Makefile:101-346` |
...
warning and the rest of Lynxer still builds (`lynxer/Makefile:19-25`).
...
| `make buildClynxer` | Full Python Lynxer PyInstaller build (`dist/clynxer`). |
| `make buildClynxerLite` | Python Lynxer "lite" build — pure-stdlib modules only (`dist/clynxer-lite`). |
...
| `make cleanAll` | Everything: `clean` + `cleanC` + `cleanLynxc` + `cleanLynxer`. |

`make cleanAll` is what the session started from. It deletes `__pycache__`,
`*.pyc`, non-stdlib `*.lynxc`, `build/`, `dist/`, `clynxer/*.so`,
`lynxer/lynxer`, the `lynxer/*.o` objects, **all `lynxer/stdlib/*.so`** and
the whole Cargo target directory (`lynxer/build/rust`) — which is why the next
...
- The binary links 12 `.o` files; the C++ stdlib modules are built by the
  ...
  `build/rust/release/liblynxer_%.so` → `cp` to `stdlib/%.so`
  (`lynxer/Makefile:87-96`).
...
`make testLynxer` → `lynxer/Makefile:101-346` runs, in order:

1. **Smoke** — `examples/hello.lynx` must print `Hello, Lynxer!` (stdin `Lynxer`).
...
**Cross-implementation parity is not part of `make testLynxer`.** The
...
**Lynxer ships 27 modules; the Python reference has 33.**

### 6.1 Lynxer modules by backend
...
Lynxer adds `network`, which does not exist in the Python tree.
...
| --- | --- | --- |
| **0 — small executable** | **Complete** | `lynxer/lynxer` builds; `--help` / `--version` work. |
...
Lynxer compiler" was based on a misreading.
...
The session began with `make cleanAll && make buildLynxer` failing. Six
...
contract says. `lynxer/scripts/check_module_contracts.py` (§8.9) now tests the
contract directly.
...
**`lynxer/rust/sqldb`** (all rusqlite 0.29 API drift)
...
**Symptom.** After fixing the compile errors, `make buildLynxer` succeeded, but
...
`...` (`lynxer/ast.cpp:566`). Any other shape is looked up in
...
**mandatory**, because `lynxer_abi`'s `export_*!` macros generate exactly the
...
- `lynxer/examples/stdlib_tui.lynx` called `global.tui.exists()` and
  ...
  (`lynxer/stdlib/tui.lynx:9,12`; `clynxer/stdlib/tui.lynx:9,22`). The fixture
  was the outlier; it was corrected. The failure mode was
  `unknown function 'global.tui.exists'`.
- `lynxer/examples/stdlib_tui.expected` listed `1` / `1` for the two
  `styleValid()` calls, but the wrapper declares `-> bool`
  (`lynxer/stdlib/tui.lynx:81`) and the Python reference returns a `bool`. It
...
- **JSON separators did not match.** `query`/`queryArgs`/`tables` used
  ...
  side embeds `native_json.hpp` into each module) and `lynxer_abi` is
...
`lynxer/scripts/check_module_contracts.py` now enforces the convention (§8.9).

### 8.9 The new contract check

`lynxer/scripts/check_module_contracts.py` tests, for every
...
2. **Packed argument bounds** — for a Rust backend, where every op uses the
   ...
   and strings are counted separately, which is how `lynxer_abi` delivers them.
   *(failure)*
...
It is wired into the first line of the `test` recipe in `lynxer/Makefile`, so
`make testLynxer` and CI both run it, and it fails with a clear message if
...
| Check | Result |
| --- | --- |
| `make buildLynxer` | passes — binary + 24 native modules |
| `make testLynxer` | passes — all 9 stages of §5, including the new contract check |
...
| Cross-implementation diff | the rewritten `sqldb` fixture produces output **identical** to `venv/bin/python clynxer/shell.py` on the same file — the strongest available evidence of parity |
...
**A note on the parity method.** Comparing Lynxer against the Python reference
...
| --- | --- |
| "Milestone 9 — compatibility gates: **not yet implemented**; no evidence of compatibility gates" | A baseline comparison exists (`todo.md:260`, 15/55 at 2026-09-13), and there **are** compatibility gates in the suite: compiled-vs-interpreted parity (`lynxer/Makefile:178-206`), `.lynxc` rejection (`:223-227`), and 25 `.expected` diffs (`:323-336`). |
| "No standalone compiler for Lynxer exists" | `--compile` / `--bundle` produce standalone ELF executables with working module imports, multi-file bundling and `--include` (`todo.md:50-76`; verified via `lynxer --help` and stage 3-4 of the suite). |
...
| "The `cli` files in `lynxer/stdlib` suggest CLI integration" | `cli` is the **user-facing command-line stdlib module**, entirely unrelated to Lynxer's own `--help`/subcommand surface in `shell.cpp`. |
...
- **test22** — **fixed.** The crash is gone, and the typed 8-byte write path no
  ...
  2^53 a double cannot represent (`lynxer/builtins.cpp`, `signedMemoryPayload` /
  `unsignedMemoryPayload`). Direction confirmed as *clynxer → lynxer*: the
  reference (`clynxer/builtins.py:83`) range-checks and stores the exact integer.
...
4. ~~Add a "does the module satisfy its own wrapper?" check to the suite.~~
   **Done** — `lynxer/scripts/check_module_contracts.py`, wired into the first
   line of `lynxer/Makefile`'s `test` recipe. It found an 18th `tui` defect that
...
whenever the file was edited. `run_source` already passed `lynxer.lua`; `eval`
now does too.
...
`LYNXER_GAME_HEADLESS=1`, so `game` tried to open a window. The fixture loop
...
`make test` (cwd is `lynxer/`) but breaks a by-hand run from the repository
...
running each fixture through `venv/bin/python clynxer/shell.py` from the same
...
the fixture's relative asset path assumes `lynxer/`. Re-run from the matching
...
| Check | Result |
| --- | --- |
| `make buildLynxer` | passes |
| `make testLynxer` | passes |
...
All 69 are already *registered* in Lynxer: `builtins.cpp` has a
`handlerTable()` of implemented built-ins and an `unsupportedTable()` whose
entries fail with `<name>() is not supported in Lynxer yet`. Porting a family
...
built-ins versus stdlib modules, even though Lynxer already provides
...
`builtins.cpp`, guarded by a `LYNXER_POSIX_BUILTINS` macro so a host without
...
(`clynxer/values.py:535`), so the value-less operations (`Close`, `Remove`,
`Rename`, `Link`, `Chmod`) must return the integer `0`, not Lynxer's `none`.
...
by a new `builtin_*` fixture glob in `lynxer/Makefile` that diffs against a
...
`builtin_*` glob in `lynxer/Makefile` that diffs against a sibling `.expected`,
...
The reference implements `sound*` on **Arcade**. Lynxer instead bridges the
...
| Lynxer | `lynxer: /tmp/fn.lynx:4:24: unknown variable 'worker'` |

So `nativeThreadStart(global.worker, [int 42])` cannot be written in Lynxer at
all: **named global functions are not first-class values**. Lynxer's value model
...
- **`Number.null` is `0`.** Every value-less operation in these families returns
  the integer `0`, not Lynxer's `none`. Getting this wrong was the only
  difference in the first `filesystem*` comparison.
- **`substring` is a Lynxer extension.** The Python reference has no such
  ...
  `-> int` on a `global` function, which Lynxer accepts.
...
| --- | --- |
| `lynxer/ast.cpp:224-520` | `nativeCallTable()` — the 32 fixed call shapes |
| `lynxer/ast.cpp:481,494,505` | The three packed (`...`) call shapes |
| `lynxer/ast.cpp:522-587` | `callNative()` — signature normalisation and dispatch |
| `lynxer/ast.cpp:560-563` | Shape lookup + `unsupported native signature` |
| `lynxer/ast.cpp:566` | `const bool packed = types.size() == 1 && types[0] == "..."` — **the packed/fixed selector** |
| `lynxer/rust/abi/src/lib.rs:199-269` | `export_int!` / `export_float!` / `export_string!` / `clynxer_module!` |

**Modules fixed and documented**

| Reference | What it is |
| --- | --- |
| `lynxer/rust/sound/src/lib.rs` | `SoundEntry { path, volume, sink }`, `SoundState { entries, output }`, `start_playback()`, all 12 ops, `OPS` table |
| `lynxer/rust/sqldb/src/lib.rs` | Path-based rewrite: `with_conn()`, `query_rows()`, `list_tables()`, `json_dumps()`, `value_to_scalar()`, `parse_params_json()`, `value_to_json()`, `OPS` table (10 ops, `open` removed) |
| `lynxer/rust/tui/src/lib.rs` | `tui_enter`, `println!("{}", "─".repeat(40))`, `OPS` table (70 ops). All 18 positional-index reads corrected; `confirm` split from `confirmDefault`; `tui_clear` flushes stdout |
| `lynxer/rust/lua/src/lib.rs` | `eval_source` sets a `lynxer.lua` chunk name |
...
| `lynxer/stdlib/tui.lynx:81` | `styleValid(...) -> bool` |
| `lynxer/rust/image/src/lib.rs:209-218` | The correct per-type indexing convention (`args.int(0)` + `args.string(0)`) |
| `lynxer/rust/json/src/lib.rs:21-50` | The `compact()` helper `sqldb`'s `json_dumps()` mirrors |
| `lynxer/ast.cpp` (`kMaxPackedArgs`) | The 64-argument total limit for packed calls |

**Build and test**

| Reference | What it is |
| --- | --- |
| `Makefile:153-156` | `buildLynxer` |
| `Makefile:70-71`, `:65-68` | `testLynxer`, `test` |
| `Makefile:195-197` | `cleanAll` composition |
| `lynxer/Makefile:15` | `RUST_MODULE_NAMES` |
| `lynxer/Makefile:87-96` | Rust crate → `stdlib/<name>.so` rule |
| `lynxer/Makefile:101-346` | The whole test suite |
| `lynxer/Makefile:178-206` | Compiled-vs-interpreted parity gates |
| `lynxer/Makefile:223-227` | `.lynxc` rejection gate |
| `lynxer/Makefile:318-322` | `--list-stdlibs` gate |
| `lynxer/Makefile:323-336` | Stdlib fixture `.expected` diffs |
| `.github/workflows/buildLynxer.yml` | CI: build + `make testLynxer` on push/PR |

**Roadmap and docs**

| Reference | What it is |
| --- | --- |
| `todo.md:3-5` | What Lynxer is |
